      ...
      >
        ...
        <AnimatePresence>
          {isWorkMenuOpen && (
            ...
            >
              {/* Bagian Atas: Background sama dengan Header (#cec9c0) */}
              <div className="bg-[#cec9c0] pt-6 flex flex-col">
                {/* Grid Kategori (Diperkecil sedikit font-nya agar muat di ukuran baru) */}
                <div className="grid grid-cols-2 gap-y-3 gap-x-8 text-[2rem] md:text-[2.25rem] tracking-tight mb-10 text-black">
                  <div className="hover:italic cursor-pointer transition-all flex items-start">
                    Website<sup className="text-xs ml-1 mt-1.5 font-sans">7</sup>
                  </div>
                  <div className="hover:italic cursor-pointer transition-all flex items-start">
                    Mobile App<sup className="text-xs ml-1 mt-1.5 font-sans">4</sup>
                  </div>
                  <div className="hover:italic cursor-pointer transition-all flex items-start">
                    Experiential<sup className="text-xs ml-1 mt-1.5 font-sans">4</sup>
                  </div>
                  <div className="hover:italic cursor-pointer transition-all flex items-start">
                    E-commerce<sup className="text-xs ml-1 mt-1.5 font-sans">3</sup>
                  </div>
                </div>
                
                {/* Folder Tab (Trapesium) */}
                <div 
                  className="bg-white inline-flex items-center gap-2 px-5 py-3 pr-10 text-[10px] font-bold tracking-widest uppercase relative top-[1px] w-max text-black"
              ...
              {/* Bagian Bawah: Daftar Proyek (Putih) */}
              <div className="bg-white w-full flex flex-col shadow-2xl pb-2">
                {FEATURED_PROJECTS.map((project) => (
                  <div key={project.id} className="group flex items-start gap-4 p-4 border-t border-black/10 hover:bg-neutral-50 transition-colors cursor-pointer">
                    {/* Gambar Proyek (Ukuran sedikit disesuaikan untuk overlay kecil) */}
                    <img 
                      src={project.image} 
                      alt={project.title} 
                      className="w-[75px] h-[75px] object-cover bg-neutral-200 shrink-0" 
                    />
                    
                    {/* Judul Proyek */}
                    <div className="w-[120px] pt-1">
                      <span className="text-[10px] uppercase font-bold tracking-wide text-black leading-tight block">
                        {project.title}
                      </span>
                    </div>
                    ...
                    {/* Kode Proyek */}
                    <div className="text-[10px] uppercase tracking-wide text-black pt-1 shrink-0 font-medium">
                      {project.code}
                    </div>
                  </div>
                ))}
                ...
                <div className="px-4 pt-4 pb-2">
                  <button className="w-full border border-black py-3 text-xs uppercase hover:bg-black hover:text-white transition-colors font-medium tracking-wide">
                    SEE ALL WORK [17]
                  </button>
                </div>
              </div>
